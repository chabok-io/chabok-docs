...
> `نکته` : به طور کلی چابک شامل ۲ محیط تست (سندباکس) و عملیاتی می‌باشد. حساب‌های رایگان چابک (تا ۳۰ هزار کاربر) بر روی محیط سندباکس و حساب‌های پریمیوم روی عملیاتی قرار می‌گیرند. کلاینت چابک به صورت پیش‌فرض بر روی حالت **تست (sandbox)** می‌باشد. برای استفاده از محیط **عملیاتی (production)** مقدار `setDevelopment` را `NO` قرار دهید. دقت داشته باشید که اگر محیط چابک را تغییر می‌دهید حتما باید اطلاعات حساب چابک (AppId, APIKey, Username و Password) را هم تغییر دهید. 

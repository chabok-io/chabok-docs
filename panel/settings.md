...
> `نکته ` : دقت داشته باشید که فیلد‌های `id`،`identifier` ،`device-type` ،`device-model` و `session-count` در ایمپورت از وان سیگنال **اجباری** می‌باشند. 

> `نکته ` : همینطور فیلد‌های `game-version`،`created-at` ،`last-active` ،`device-os` و `tags` در ایمپورت از وان‌سیگنال **اختیاری** می‌باشند.

> `نکته ` : از آن جایی که **کد تگ‌ها** در وان‌سیگنال به صورت `key/value` می‌باشند اما در چابک به صورت `key`، در صورت ایمپورت، تگ‌ها به صورت `key-value` ذخیره می‌شوند.

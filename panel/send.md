...
به طور کلی پیام‌ها در چابک از **۳** طریق (کانال) ارسال می‌شوند. این ۳ طریق عبارتند از **صندوق پیام** (از سوکت‌های مخصوص چابک)، **پوش‌نوتیفیکیشن** (از سرویس‌های گوگل، اپل و مایکروسافت) و **پیامک.** 

از این رو، سیاست‌های ارسال بر اساس این ۳ طریق (به صورت مجزا یا ترکیبشان) ایجاد شده‌اند. از بین **۴** سیاست ارسال چابک، ۲تا از آن‌ها ترکیبی هستند (صندوق پیام و هشمند).
...
- یا در استفاده پیچیده‌تر می‌توانید **اطلاعات یک رفتار** را سفارشی کنید:
 ...
 مثلا کاربر یک آیتمی را به **سبد خرید خود افزوده است** و شما می‌خواهید هر کاربر را یادآوری کنید که آیتمشان هنوز موجود است و هنوز فرصت دارند خرید را تکمیل کنند. در این حالت بهتر است **نام آیتمی** که به سبد خرید هر کاربر اضافه شده است برایشان ارسال شود تا با شناخت آن انگیزه بیش‌تری برای خرید داشته‌باشند.
...
اگر بخواهید از فیلتر **date** در شخصی‌سازی محتوای پیام خود استفاده کنید باید نمونه فرم زیر را بنویسید.
...
اگر **date** به تاریخ شمسی باشد، باید فرم زیر را به متن پیام خود اضافه کنید.
...
در صورتی که قصد دارید عدد به کاربران نمایش دهید باید از فیلتر `locale` استفاده کنید و به شکل زیر، آن را بنویسید.

```
{{abc | locale("fa-IR")}}
```
و در نتیجه ۲۰۰,۰۰۰ را به کاربران نمایش می‌دهد.

-`join`

اگر بخواهید آرایه‌ای از رشته را بنویسید و با نمادی مثل **,** آن‌ها را از هم جدا کنید باید از فیلتر **join** استفاده کنید. 

```
{{abc | join(', '))}}
```
که در آخر چیزی که به کاربر نمایش می‌دهد به شکل زیر خواهد بود:
  
**[a,b,c,d]**
 
>`نکته`: منظور از **abc**، متن پیامی است که به کاربران ارسال می‌شود.
...
**اندروید**: از اینجا رنگ LED گوشی را در هنگام دریافت نوتیفیکیشن انتخاب کنید و آیکون کوچک و بزرگ (در فرمت PNG یا JPEG) در نوتیفیکیشن بگذارید.

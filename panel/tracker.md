...
در **لینک ترکر** می‌توانید خودتان جداگانه پارامترهای ‍‍‍‍‍‍‍‍`fallback` ،`redirect_ios` ،`redirect_android`، `campaign`، `adgroup` ،`creative`  `click_id` ،`android_id` را مانند زیر قرار دهید:

...
A/B testing یا split testing  یک تکنیک مارکتینگ است برای انتخاب بهتر بین دو نمونه (الف و ب). بدین صورت که زمانی که بین یک المان در دو نمونه خود شک دارید، هر دو را آزمایش می‌کنید و هر کدام که تاثیری بیش‌تری داشت را انتخاب می‌کنید.
تاثیر این انتخاب‌ها را معیارهایی مانند نرخ تبدیل، نرخ کلیک، ارسال فرم‌ها و ... تعیین می‌کنند.
...
با توجه به گران بودن فرایند جذب هم باید تعامل سازنده‌ای با کاربران موجود داشته باشید و هم برای بازگشت سرمایه جذب خود برای کابران جدید برنامه تعامل بریزید.

#### Ad Network
شبکه‌های تبلیغاتی گروه‌های هستند که نقش رابط را بین تبلیغ دهنده و تبلیغ گیرنده (ناشرین) ایفا می‌کنند. بیش‌تر شبکه‌های تبلیغاتی فرمت‌های مختلفی از تبلیغات را مانند بنر‌ها و یا تبلیغات محلی (native ads) پشتیبانی می‌کنند. البته ممکن است بعضی هم روی فرمت خاصی مانند ویدیو تمرکز داشته باشند.

#### API (Application Programming Interface)

امکان به اشتراک گذاشتن یا در اختیار گذاشتن بخشی از یک نرم‌افزار با برنامه‌نویسان دیگر است. بدین ترتیب که می‌توان دسترسی محدودی از نرم‌افزار را ایجاد کرد تا بقیه از آن بهره ببرند. ایجاد کننده‌های API کمک می‌کنند تا اپلیکیشن‌ها کارکرد خود را توسعه دهند یا به دیتای جدیدی دسترسی داشته باشند. 

####  ASO (App Store Optimization)

استراتژی است که کمک می‌کند تا اپلیکیشن در استورها بیش‌تر دیده شود. استورها هر اپلیکیشن را بر اساس فاکتورهای مختلفی رتبه‌بندی می‌کنند. با بهره‌گیری از کلمات کلیدی بهینه، تصاویرمفید، توضیحات دقیق می‌توانید رتبه و در نتیجه نصب‌های خود را بالا ببرید. 
...
اتریبیوشن موفق به خوبی فرایند تبدیل (conversion) را در قیف مارکتینگ پوشش می‌دهد، یعنی از تعامل با کمپین تبلیغاتی تا نصب اپلیکیشن و رصد رفتار کاربران جذب شده. بنابراین نقش آن در بهینه‌سازی کلیدی است. 

#### Attribution Window
محدوده زمانی قابل قبول بین تعامل با تبلیغ (کلیک یا مشاهده) و نصب است، که ناشر از پیش تعیین می‌کند. در صورت افتادن نصب در این محدوده ناشر نصب را تایید شده و مربوط به کمپین تبلیغاتی خود خواهد دانست.

####  Analytics

آنالینتیکس واژه‌ای است که تمام کارکردهای تحلیل، جمع‌آوری و ارائه داده‌های اپلیکیشن را در بر می‌گیرد. زمانی اپلیکیشن منتشر می‌شود، برای مارکتر حیاتی است که بداند چه اتفاقاتی در اپلیکیشن رخ می‌دهد.
...
#### Backlinks
هایپرلینک‌هایی هستند در وبسایت‌های دیگر که به سایت شما می‌روند. بک لینک دادن یک فرایند ضروری مارکتر در ساختن لینک است که نقش مهمی در SEO وبسایت را ایفا می‌کند.
...
کال‌بک‌های آنی (real-time) مداوم به کسب و کارها اطمینانی از دقت و صحت داده‌های خود می‌دهد که اجازه می‌دهد با خیال راحت با کاربران تعامل مناسب داشته باشند.
...
همه کسب و کارها این را خواهند داشتند اما آنچه اهمیت دارد آنالیز این نرخ و در نتیجه کاهش آن است. سازندگان اپلیکیشن با آگاهی از این آمار با تغییراتی در تجربه کاربری (UX)، قیمت محصولات، بهینه سازی امکانات و تعاملات درون اپلیکیشن سعی در جبران و کاهش نرخ دارند.
...
#### Cohort
دسته‌ای از کاربران که ویژگی مشترکی دارند. این ویژگی می‌تواند هر چیزی باشد؛ از موقعیت مکانی، تا کاربرانی که در یک زمان نصب را انجام داده‌اند.
دسته‌بندی کاربران مقایسه داده‌ها را (براساس یک شاخص) آسان می‌کند.
...
با داشتن نرخ تبدیل مارکتر یک معیاری دارد که با آن می‌تواند رشد را افزایش دهد.

#### Cost Models (CPC, CPI, CPA)
مدل‌های پرداختی تبلیغات موبایل است. این مدل‌ها بر پایه هزینه براساس (cost per) یک چیزی است (مشاهده، کلیک، نصب، اکشن یا رفتار).


#### CTIT (Click to Install Time)

زمان بین کلیک کاربر روی تبلیغ و باز کردن اپلیکیشن مربوط برای بار اول است.
...
خیلی مهم است که مارکتر‌ها دید سفر را به تعاملات کاربران خود داشته باشند. این دید تصویرسازی شما از مسیر و مقصد کاربر را تقویت می‌کند و به کسب و کار شما داده‌های کلیدی مانند آگاهی از جاهایی و یا دلایلی که کاربرانی از شما جدا شده‌اند  می‌دهد. بدیهی است که همه کاربران جذب شده با شما نخواهند ماند؛ پس چه بهتر که با استفاده از رفتارهای آنان بتوانید تجربه بهتری برای بقیه فراهم کنید.
...
#### Deep Linking
هر موقع روی لینکی زدید و به صفحه‌ خاصی از اپلیکیشن هدایت شدید، از دیپ لینک استفاده کرده‌اید. به عبارت دیگر دیپ لینک به شما امکان می‌دهد به جای لینک دادن به صفحه پیش‌فرض یا خانه خود بتوانید کاربرتان مستقیما به صفحه دلخواه و مرتبط با محتوا هدایت کنید. مثلا با کلیک روی یک بنر، کاربر مستقیم به صفحه خرید یک محصول خاص برود. 
...
در موبایل مارکتینگ هر تعامل، برخورد یا واکنش کاربر با اپلیکیشن  (یا تبلیغ) را انگیجمنت می‌گویند. این تعامل هرچیزی از باز کردن اپلیکیشن تا خرید یک محصول، رزرو بلیت می‌تواند باشد. 
یکی از بزرگ‌ترین هدف‌های مارکتر است که می‌تواند پایه مطمئنی برای کسب و کار شما باشد.


#### Event

در چابک و کلا موبایل مارکتینگ این واژه خیلی زیاد به کار برده می‌شود. ایونت یا رویداد هر اتفاقی (یا تعامل) که بین کاربر و اپلیکیشن است. از این مفهوم به عنوان رفتار درون اپلیکیشن کاربر هم ذکر می‌شود. رصد رویدادها شما را از رفتار هر کاربری آگاه می‌سازد.
...
نکته مثبت تحلیل فانل این است که به شما می‌کند سوال‌های کلیدی زیر را پاسخ دهید:

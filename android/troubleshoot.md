...
در صورتی که شما دستگاه را با `userId` که ثبت کردید **در بخش مشترکین پنل مشاهده نکردید** به نکات زیر توجه فرمایید:
 ...
 ب- **تعیین درست محیط چابک:** متد `configureEnvironment` را با توجه به محیط عملیاتی یا آزمایشی پیاده‌سازی کنید.
...
<p class="text-center">در صورت مشاهده دستگاه کاربر در بخش مشترکین پنل، از عضویت کاربر در کانالی که با آن پیام خود را ارسال می‌کنید اطمینان یابید. برای اینکه کانال‌هایی که کاربرتان در آن‌ها عضو هستند را مشاهده می‌کنید، می‌توانید روی آیکون ![کانال‌ها](http://uupload.ir/files/24jn_channels.png)</p> در کارت هر دستگاه در بخش مشترکین پنل کلیک کنید. علاوه بر آن شما می‌توانید از اتصال یا عدم اتصال دستگاه از مشاهده **نوار سبز رنگ** در بالای کارت مطلع شوید.
...
خطای  SENDER_ID_MISMATCH زمانی نمایش داده می‌شود که این دو فایل با هم تطابق نداشته باشند. به معنای بهتر، این خطا درست زمانی اتفاق می‌افتد که فایل اشتباهی را از پنل فایربیس دانلود کردید و در کنار ماژول اصلی پروژه خود قرار دادید و یا اطلاعات اشتباه از پنل فایربیس را در پنل چابک قرار دادید.

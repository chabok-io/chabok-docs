...
در صورتی که شما دستگاه را با `userId` که ثبت کردید **در بخش مشترکین پنل مشاهده نکردید** به نکات زیر توجه فرمایید:
 ...
 ب- **تعیین درست محیط چابک:** متد `configureEnvironment` را با توجه به محیط عملیاتی یا آزمایشی پیاده‌سازی می‌شود.

 پ- **با توجه به محیط عملیاتی، فایل را دانلود و در جای مناسبی کپی کند.**
...
#####۶- همیشه مطمئن شوید که فایل **google-services.json** پروژه را که از <a href="https://console.firebase.google.com/"> پنل فایربیس </a> دانلود کردید در پوشه ماژول اصلی پروژه قرار داده باشید.

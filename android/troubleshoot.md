...
- وی‌پی‌ان دستگاه خاموش باشد.

- اطمینان از تعیین درست محیط چابک (آزمایشی و عملیاتی) توسط متد `setDevelopment`

- اطمینان از وارد کردن صحیح اطلاعات حساب(`appId`, `APIKey`, `username`, `password`)
...
#### ۲- تنظیم صحیح پوش‌نوتیفیکیشن برای هر پلتفرم
...
در صورت مشاهده متن **غیرفعال** در کارت تنظیمات هر پلتفرم دستگاه به بخش مشترکین اضافه نشده است. برای مثال اگر تنظیمات پوش‌نوتیفیکیشن اندروید را به درستی انجام داده باشید ولی دستگاه اندروید‌ی به بخش مشترکین اضافه نشده باشد بر روی کارت تنظیمات پوش‌نوتیفیکیشن اندروید متن غیرفعال را مشاهده خواهید کرد.

#### ۳- یکسان بودن `senderId` و `appId` در پنل و متد `init` در اندروید
...
#### ۴- تعریف GCMReceiver در اندروید

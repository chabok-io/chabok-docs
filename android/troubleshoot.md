...
اگر Sender ID داخل فایل google-services.json وجود داشته باشد. به این معناست که این فایل از داخل پنل فایربیس دانلود شده و در صورتی که Sender ID داخل پنل چابک باشد، یعنی از روی پنل فایربیس کپی شده است.

خطای  SENDER_ID_MISMATCH زمانی نمایش داده می‌شود که این دو فایل با هم تطابق نداشته باشند،. به معنای بهتر این خطا درست زمانی اتفاق میافتد که فایل اشتباهی را از پنل فایربیس دانلود کردید و در کنار پروژه ماژول قرار دادید و یا اطلاعات اشتباه از پنل فایربیس را در پنل چابک قرار دادید.

...
> `نکته:` دقت داشته باشید که **قابلیت آنی (realtime)**  چابک از نسخه ۳ به بالا به صورت پیش فرض **غیر فعال** است. برای فعال کردن مقدار قابلیت آنی (realtime)، کافی است مقدار پیش‌فرض آن را در فایل تنظیمات چابک (**Chabok.sandbox.json**  یا  **Chabok.production.json**) که از پنل دانلود می‌کنید تغییر دهید.

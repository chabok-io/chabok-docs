...
- افزودن متد `setAutoResetBadge` برای رست خودکار **Badge** و پاک کردن نوتیفیکیشن‌ها

> ` نکته:` مقدار پیش‌فرض این متد `true` است، بنابراین اگر مقدار `false` را قرار دهید برای رست دستی **Badge** باید متد `resetBadge` را فراخوانی کنید.

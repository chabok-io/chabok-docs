...
>`نکته`: غیرفعال سازی نوتیفیکیشن و حذف توکن به کمک متد `deleteInstanceId` [فایربیس](https://firebase.google.com/docs/reference/android/com/google/firebase/iid/FirebaseInstanceId) در نسخه‌های ۳.۱.۲ به پایین چابک، می‌تواند باعث نامعتبر بودن توکن چابک شود و روی توکن پوش تاثیر منفی بگذارد پس استفاده از آن توصیه نمی‌شود.

>`نکته`: حذف توکن در نسخه‌ ۳.۱.۲ به بالا چابک به طور خودکار هندل می‌شود و مشکلی برای توکن پیش نمی‌آورد.

...
>`نکته`: حذف توکن به کمک متد `deleteInstanceId` [فایربیس](https://firebase.google.com/docs/reference/android/com/google/firebase/iid/FirebaseInstanceId) در نسخه‌های ۳.۱.۳ به پایین چابک، می‌تواند باعث نامعتبر شدن توکن پوش شود و در شمارش تاثیر بگذارد، پس استفاده از آن توصیه نمی‌شود.
, حذف توکن در نسخه‌ ۳.۱.۳ به بالا چابک به طور خودکار هندل می‌شود و مشکلی برای توکن پیش نمی‌آورد.

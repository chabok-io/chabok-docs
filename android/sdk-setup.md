...
به طور کلی چابک از **دو متد** برای ثبت کاربر استفاده می‌کند. **متد اول پس از عضویت و ایجاد حساب کاربری است** که شما اطلاعاتی را از کاربرتان کسب کرده‌اید. این متد `register` نام دارد. **متد دوم قبل از عضویت است** که کاربر حسابی را ایجاد نمی‌کند و شما کاربر را به عنوان مهمان می‌شناسید. این متد `registerAsGuest` نام دارد.


#### ثبت کاربر قبل از عضویت در اپلیکیشن یا مهمان (Register As Guest)

اگر اپلیکیشن شما قابلیت  **ایجاد حساب کاربری** دارد، می‌توانید کاربر را تا زمانی که حساب ایجاد نکرده است به عنوان  **کاربر مهمان**  در سیستم خود ثبت کنید و سپس به محض ایجاد حساب و دریافت اطلاعات او، آن کاربر را به عنوان [**کاربر دائم**](/android/sdk-setup.html#ثبت-کاربر-پس-از-عضویت-در-اپلیکیشن-register-after-user-sign-up) خود مانند پایین ثبت کنید. 
...
متد زیر کاربر را به عنوان کاربر مهمان ثبت می‌کند و به طور خودکار یک تگ مهمان (CHABOK_GUEST) به او اختصاص می‌دهد. دقت داشته باشید که **این متد را به تنهایی به کار نبرید** زیرا با هر بازدید کاربر مهمان جدید ثبت می‌شود. برای اطلاعات بیشتر مستندات [ترکر نصب](/android/tracker.html) را مطالعه کنید.

```java
AdpPushClient.get().registerAsGuest();
```

#### ثبت کاربر پس از عضویت در اپلیکیشن (Register After User Sign up)

در این متد شما می‌توانید کاربر را پس از ایجاد حساب با یک شناسه منحصر به فرد ثبت کنید.
...
به عنوان مثال اگر اپلیکیشن شما صفحه **ورود** و **ثبت‌نام** دارد، متد `register` را در صفحه **ورود** یا **ثبت‌نام** پس از **احراز هویت کاربر** و همچنین، پس از هر بار اجرای اپلیکیشن (در کلاس `Application` متد `onCreate`) فراخوانی کنید تا کاربر به سرور چابک متصل شود.
...
برای حذف دستگاه کاربر از سرور چابک می‌توانید از متد `unregister` استفاده کنید. پس از حذف کاربر، چابک دیگر به دستگاه‌های آن `userId` پوش ارسال نخواهد کرد. توصیه می‌شود این متد را زمانی که کاربر در اپلیکیشنتان از حساب خود خارج می‌شود (**Logout**) فراخوانی کنید. این امر باعث می‌شود تا کاربر از حفظ شدن حریم شخصی خود پس از خروج از حساب کاربری اطمینان یابد. پس از آن هم می‌توانید کاربر را به عنوان یک کاربر مهمان `register` کنید تا همچنان با او تعامل داشته باشید.

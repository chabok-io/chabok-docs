...
در ابزار جدید چابک، امکان دریافت موقعیت مکانی کاربر فراهم شده است. برای این منظور لازم است از شیء `LocationManager` استفاده نمایید. برای مقداردهی اولیه متد `init` را با context موردنظر فراخوانی نمایید، به شکل زیر:
...
### دریافت موقعیت مکانی
برای دریافت موقعیت مکانی بصورت مداوم باید متد `startLocationUpdates`  را با پارامتر مربوط که در ادامه توضیح داده می‌شود، صدا بزنید:
...
>`نکته:`
>درصورتی که حالت background توسط متد disableBackgroundMode غیرفعال شده باشد، با فراخوانی متد `stop` سرویس متوقف خواهدشد، بصورت پیش‌فرض حالت background‌ غیرفعال می باشد.
...
```
با این کار، بروزرسانی موقعیت مکانی را در سرویس خود LocationHostService دریافت خواهید نمود، برای جزيیات این موضوع به [تنظیمات مکان‌یابی](location-config.html) مراجعه کنید.
...
توجه کنید که این موقیعت مکانی لزوما موقعیت مکانی جاری نمی‌باشد.
...
```
وقتی حالت background غیرفعال باشد، با فراخوانی متد `stop` سرویس متوقف خواهد شد و درحالت kill‌ بودن برنامه سرویس مکانی فراخوانی نمی‌شود و شما به اطلاعات مکانی دسترسی نخواهید داشت.

>`نکته:`
>برای جزيیات این موضوع به [تنظیمات مکان‌یابی](location-config.html) مراجعه کنید.

...
> `نکته` : دقت داشته باشید  **type** مقداری که به `value` در متد `track` داده‌اید را نمی‌توانید تغییر دهید. برای مثال اگر `boolean` ذخیره کرده‌اید، دیگر نمی‌توانید عدد یا `string` دهید.
